...
# Script to automatically create Pull Requests with GitHub API
# Requirements: GitHub Personal Access Token

echo "🚀 Starting automatic Pull Request creation with GitHub API..."
...
if [ -z "$GITHUB_TOKEN" ]; then
    ...
    echo "📝 Export token: export GITHUB_TOKEN=your_token_here"
    exit 1
fi

# Function to create PR with API
create_pr_api() {
    local branch=$1
    local title=$2
    local description=$3
...
EOF
    ...
    if echo "$response" | jq -e '.html_url' > /dev/null 2>&1; then
        ...
        echo "❌ Error creating PR: $(echo "$response" | jq -r '.message // "Unknown error"')"
    fi
...
PR1_DESC="## 🎯 Mục tiêu
Sửa lỗi TypeError và cải thiện logic ứng dụng tournament

## ✨ Tính năng mới
- Thêm comprehensive null/array checks
- Cải thiện error handling
- Sửa logic tournament rules

## 🔧 Sửa lỗi
- Fix 'Cannot read properties of undefined' errors trong multiple functions
- Sửa logic bracket inconsistencies
- Cải thiện player management logic
...
create_pr_api "fix/type-errors-and-logic-improvements" "$PR1_TITLE" "$PR1_DESC"
...
create_pr_api "feature/group-layout-and-styling" "$PR2_TITLE" "$PR2_DESC"
...
create_pr_api "feature/random-score-generation" "$PR3_TITLE" "$PR3_DESC"
...
create_pr_api "feature/simplified-knockout-stage" "$PR4_TITLE" "$PR4_DESC"
...
create_pr_api "refactor/component-architecture" "$PR5_TITLE" "$PR5_DESC"
...
create_pr_api "fix/css-conflicts-and-styling" "$PR6_TITLE" "$PR6_DESC"
...
create_pr_api "feature/player-profile-modal" "$PR7_TITLE" "$PR7_DESC"
...
create_pr_api "feature/keyboard-shortcuts" "$PR8_TITLE" "$PR8_DESC"
